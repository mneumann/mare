  ...
  class Visitor < Mare::AST::Visitor
    ...
    def resolve(ctx : Context, info : Infer::Info) : Span
      @analysis[info]? || begin
        ...
        if lazy_type_params.any?
          ...
          lazy_type_params.reduce(span) { |span, type_param|
            type_param_span = lookup_type_param_partial_reified_span(ctx, type_param)

            pp span.combine_mt(type_param_span) { |mt, type_param_mt|
              mt.substitute_lazy_type_params({ type_param => type_param_mt }, max_depth)
            }

            # if type_param_cap_mt
            #   type_param_span = type_param_span
            #     .deciding_type_param(type_param, type_param_cap_mt)
            #     .not_nil!
            # end


            # pp span.transform_mt_using(type_param) { |mt, type_param_cap_mt|
            ...
            #   raise NotImplementedError.new("non-terminal type param bound span here") \
            #     unless type_param_span.inner.is_a?(Span::Terminal)

            #   type_param_mt = type_param_span.inner.as(Span::Terminal).meta_type
    ...
    def deciding_type_args_of(ctx : Context, rt : Infer::ReifiedType, raw_span : Span) : Span
      ...
      .reduce(raw_span) { |span, (type_param, type_arg)|
        ...
        #   type_arg_type_param = type_arg.inner.as(MetaType::Nominal).defn.as(Infer::TypeParam)
        #   type_arg_type_param_span =
        #     lookup_type_param_partial_reified_span(ctx, type_arg_type_param)
        #       .transform_mt(&.cap_only)
        #   type_arg_type_param_span.combine_mt_to_span(span) do
        #   span.combine_mt(type_arg_type_param_span) do |
